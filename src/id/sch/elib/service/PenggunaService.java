 ...
 * @author rizky.aditya
 */
public class PenggunaService implements BaseServiceInterface{
    
    private final GrailsRestClient grc = new GrailsRestClient();
    private final Gson gson = new Gson();
    private final String endpoint = "pengguna";

    @Override
    public Object listAll() {
        String json = grc.get(endpoint);
        Type type = new TypeToken<List<Pengguna>>() {
        }.getType();
        List<Pengguna> output = gson.fromJson(json, type);
        return output;
    }

    @Override
    public Object get(long id) {
        Pengguna jo = gson.fromJson(grc.get(endpoint + "/" + id), Pengguna.class);
        return jo;
    }
    ...
    public Object search(Object obj) {
        ...
        ArrayList<Pengguna> list = gson.fromJson(response, type);
        return list;
    }
