 ...
 * @author rizky.aditya
 */
public class DendaService implements BaseServiceInterface{

    private final GrailsRestClient grc = new GrailsRestClient();
    private final Gson gson = new Gson();
    private final String endpoint = "denda";
    
    @Override
    public Object listAll() {
        String json = grc.get(endpoint);
        Type type = new TypeToken<List<Denda>>() {
        }.getType();
        List<Denda> output = gson.fromJson(json, type);
        return output;    
    }

    @Override
    public Object get(long id) {
        Denda jo = gson.fromJson(grc.get(endpoint + "/" + id), Denda.class);
        return jo;
    }
    ...
    public Object search(Object obj) {
        ...
        ArrayList<Denda> list = gson.fromJson(response, type);
        return list;
    }
